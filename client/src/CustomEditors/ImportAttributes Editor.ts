 ...
 */
export class ImportattributesEditorProvider implements vscode.CustomTextEditorProvider {

	public static register(context: vscode.ExtensionContext): vscode.Disposable {
		const provider = new ImportattributesEditorProvider(context);
		const providerRegistration = vscode.window.registerCustomEditorProvider(ImportattributesEditorProvider.viewType, provider);
		return providerRegistration;
	}

	private static readonly viewType = 'MarcelSotiropoulos.Importattributes';
	...
	private getHtmlForWebview(webview: vscode.WebviewPanel, textdocument :vscode.TextDocument): string {
		...
		for (let index = 0; index < textdocument.lineCount; index++) {
			...
			if(line.text.trim().length > 0 && line.text.search(/^156\t[0-9]+\t/gm) >= 0) {
		...
		return /* html */`
			...
			<head>
				...
				-->
				<link rel="stylesheet" href="${createWebViewLink(this.context.extensionPath, webview, "webview", "css", "main.css")}">
			...
			${html}
				...
				<script src="${createWebViewLink(this.context.extensionPath, webview, "webview", "js", "jquery.datatables.js")}"></script>
				<script>
